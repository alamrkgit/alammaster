...
function InggrisTgl2($tanggal){
	$tgl=substr($tanggal,8,2);
	$bln=substr($tanggal,5,2);
	$thn=substr($tanggal,0,4);
    $h = substr($tanggal,11,2);
    $i = substr($tanggal,14,2);
    $s = substr($tanggal,17,2); 
	$tanggal="$tgl-$bln-$thn $h:$i:$s";
	return $tanggal;
}

function InggrisTgl3($tanggal){
	$tgl=substr($tanggal,0,2);
	$bln=substr($tanggal,3,2);
	$thn=substr($tanggal,6,4);
    $h = substr($tanggal,11,2);
